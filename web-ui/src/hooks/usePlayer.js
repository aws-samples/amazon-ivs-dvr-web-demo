...
const usePlayer = (urlToLoad, type, isChannelLive) => {
  ...
  const onStateChange = useCallback(() => {
    ...
    console.log(`${type.toUpperCase()} Player State - ${newState}`);
  }, [type]);
  ...
  const onError = useCallback(
    (err) => {
      console.warn(
        `${type.toUpperCase()} Player Event - ERROR:`,
        err,
        playerRef.current
      );
  ...
  }, [isChannelLive, isInitialLoading, type, urlToLoad]);
